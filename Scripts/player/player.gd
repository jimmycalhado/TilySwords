...
@onready var sprite: Sprite2D = $Jogador
...
signal meat_collector(value:int)

func _ready():
	GameManage.player = self
	meat_collector.connect(func(value: int): 
		GameManage.meat_counter += 1
		)
...
func update_hitbox_damage(delta: float) -> void:
	...
	for body in bodies:
		if body.is_in_group("enemies"):
			var damage_amaount = 1
			damage(damage_amaount)
...
func heal(amount: int) -> int:
	...
	if health > max_health:
		health = max_health
	print("Player recebeu vida de ", amount, ". A vida total é de ", health, "/", max_health)
	return health
